...
Noise[[inoise]] <- list()
Noise[[inoise]]$Name = "Gaussian"
Noise[[inoise]]$arima = list(order=c(0,0,0))  ##uncorrelated noise
Noise[[inoise]]$df=NULL ##Gaussian

inoise <- inoise +1

Noise[[inoise]] <- list()
Noise[[inoise]]$Name = "t10"
Noise[[inoise]]$arima = list(order=c(0,0,0))  ##uncorrelated noise
Noise[[inoise]]$df=10 ##student-t df=10

inoise <- inoise +1

Noise[[inoise]] <- list()
Noise[[inoise]]$Name = "GaussAR1"
Noise[[inoise]]$arima = list(order=c(1,0,0), ar=0.3)  ##AR(1) correlation 0.3
##order=c(p,0,q) AR(p) MA(q); so this is AR(1) with coefficient 0.3
Noise[[inoise]]$df=NULL ##Gaussian

inoise <- inoise +1

Noise[[inoise]] <- list()
Noise[[inoise]]$Name = "t10AR1"
Noise[[inoise]]$arima = list(order=c(1,0,0), ar=0.3)  ##AR(1) correlation 0.3
Noise[[inoise]]$df=10 ##student-t df=10
...
rSimu <- function(sim, arima.model,df=NULL){

    x <- rep(sim$mu, sim$Lg) 
  if(is.null(df)){
    x <- x + arima.sim(arima.model,n=length(x))*sim$sigma
  }else{
    if(df>2){
	sd.fac=sqrt( (df-2)/df) ##normalise noise to variance 1 prior to multiplication by sim$sigma
    }else{
        sd.fac=1
	}		
    x <- x + arima.sim(arima.model,n=length(x),rand.gen=rt,df=df)*sd.fac*sim$sigma   
  }
  return(x)
}

for(sim in Simu){
  x <- rSimu(sim, arima.model=Noise[[1]]$arima, df=Noise[[1]]$df)
...
We now simulate a number (nb_simu_per_dataset) profile per dataset and store them in RData.
```{r simulation_save}
set.seed(100)
storeSim <- function(sim, noise, ns=nb_simu_per_dataset){
  ...
  if(!file.exists(file_sim)){
    data_sim <- replicate(ns, rSimu(sim, noise$arima, noise$df))
...
for(sim_ext in Simu_Ext){
  lapply(sim_ext, FUN=storeSim, noise=Noise)
}}
...
library(parallel)
one_simu_seg <- function(sim, method, noise){
  file_sim <- paste0("Simu_Profile/Error=", noise$Name, "_", gsub(",[ ]", "_", sim$Name), ".RData")
  file_seg <- paste0("Seg_Approach/Error=", noise$Name, "_", gsub(",[ ]", "_", sim$Name),  "_", method$Name, ".RData")
...
for(noise in Noise){
  for(sim_ext in Simu_Ext){
    for(met in Seg.method){
      lapply(sim_ext, FUN=one_simu_seg, method=met, noise=noise)
}}}
