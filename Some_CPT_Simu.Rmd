...
Simu_Ext <- lapply(Simu, FUN=function(sim_, alphas_=c(1, 2)){ #alphas_=c(1, 2, 4, 10)){
...
```{r PELT_MBIC}
 ...
 library(changepoint)
 Seg.method[[i]] <- list()
 Seg.method[[i]]$Name <- "Plt.mbic.cor"
 Seg.method[[i]]$FUN <- function(x){
...
library(capushe)
Seg.method[[i]] <- list()
Seg.method[[i]]$Name <- "Fpop-Fpsn"
Seg.method[[i]]$FUN <- function(x){
  n <- length(x)
  penalty <- 2*log(length(x))
  res <- Fpop(x, lambda=penalty)
  Kmax <- 4*length(res$t.est)
  Kboundup <- trunc(n/2) - 1
  Kbounddw <- 100

  Kmax <- min(max(Kmax, Kbounddw), Kboundup)
...
Seg.method[[i]]$FUN <- function(x){
  ...
  dataCapushe <- dataCapushe[dataCapushe$name <= max(100, n/2 - 2), ]
...
library(fpop)
Seg.method[[i]] <- list()
Seg.method[[i]]$Name <- "BS-2log(n)"
Seg.method[[i]]$FUN <- function(x){
  n <- length(x)
 
  
  res <- multiBinSeg(x, Kmax=n-1)
  J.est <- c(0, cumsum(res$J.est)) + 2*log(n)*(1:n)
  k <- which.min(J.est)
  
  if(k > 2){
    t.est <- c(sort(res$t.est[1:k]), n)
  } else { t.est <- n}
  
  
  return( list(cpt=t.est) )
}
```
...
Seg.method[[i]]$FUN <- function(x){
 ## first load quantile if available if not run simu
 n <- length(x)
 file.quantile <- paste("Simu_FDRseg/fdrseg_qtl", n, ".RData", sep="")
 if(!file.exists(file.quantile)){
   alpha <- 0.1
   r = round(50/min(alpha, 1 - alpha))
...
i <- i+1
Seg.method[[i]] <- list()
Seg.method[[i]]$Name <- "AR1seg-100"
Seg.method[[i]]$FUN <- function(x){
  res=AR1seg_func(x, Kmax=100, rho=TRUE)
  a=c(1, res$PPSelectedBreaks[1:(res$PPselected-1)]+1)
  b=res$PPSelectedBreaks[1:(res$PPselected)]
  return( list( cpt=as.vector(b)))
}
...
i <- i+1
Seg.method[[i]] <- list()
Seg.method[[i]]$Name <- "cumSeg"
Seg.method[[i]]$FUN <- function(x){
  res <- jumpoints(x, k=100)
  return( list( cpt=c( which(diff(res$fitted.values)!=0), length(x)) ) )
}
...
for(sim_ext in Simu_Ext){
    ...
    colnames(data_to_show)[1] <- "Met"
     ...
     essai <- kable(data_to_show, row.names = FALSE,  align="c", format="html", 
              ...
              row_spec(c(1, 10, 11), bold = T, color = "white", background = "#00FF00") %>%
              row_spec(c(3, 6, 7), bold = T, color = "white", background = "#A9F5A9")
     print(essai)
  }
...
for(sim_ext in Simu_Ext){
    ...
    colnames(data_to_show)[1] <- "Met"
     ...
     essai <- kable(data_to_show, row.names = FALSE,  align="c", format="html",
              ...
              add_header_above(c(" " = 1, "Time" = nb_lg, "MSE" = nb_lg, "E(K=K*)" = nb_lg, "ARI" = nb_lg)) %>%
              column_spec((1:nb_so-1)*nb_lg+1, border_right = T) %>%
              row_spec(c(1, 10, 11), bold = T, color = "white", background = "#2E2EFE") %>%
              row_spec(c(3, 6, 7), bold = T, color = "white", background = "#A9BCF5")
     print(essai)
  }
...
for(sim_ext in Simu_Ext){
    ...
    colnames(data_to_show)[1] <- "Met"
     ...
     essai <- kable(data_to_show, row.names = FALSE,  align="c", format="html",
              ...
              add_header_above(c(" " = 1, "Time" = nb_lg, "MSE" = nb_lg, "E(K=K*)" = nb_lg, "ARI" = nb_lg)) %>%
              column_spec((1:nb_so-1)*nb_lg+1, border_right = T)  %>%
              row_spec(c(1, 10, 11), bold = T, color = "white", background = "#D7261E") %>%
              row_spec(c(3, 6, 7), bold = T, color = "white", background = "#F5A9A9")
     print(essai)
  }
```
