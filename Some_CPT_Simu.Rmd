...
#Seg.method[[i]]$FUN <- function(x){
#  n <- length(x)
#  res <- CROPS.FPOP(x, min=2,max=10*log(length(x)))
...
sim <- Simu_Ext[[2]][[1]]
i <- 1
while(i <= length(Seg.method)) {
  one_load_aggregate(sim, Seg.method[[i]], "Gauss")
  i <- i+1
}
...
for(sim_ext in Simu_Ext){
    ...
    colnames(data_to_show)[1] <- "Met"
     ...
     essai <- kable(data_to_show, row.names = FALSE,  align="c", format="html", caption=paste0(sim_ext[[1]]$Name, ", Gauss") ) %>%
             ...
             row_spec(c(6, 11, 13), bold = T, color = "white", background = "#D7261E")
     print(essai)
  }
